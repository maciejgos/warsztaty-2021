$("select").on("changed.bs.select",
    function (e) {
        ...
        if ($(this).val().length == 0) {
            $("div").filter("[class~='mentor']").show();
        }
        else {
            $("div").filter("[class~='mentor']").not('.' + $(this).val().join(',.')).hide();
            $("div").filter("[class~='mentor']").filter('.' + $(this).val().join(',.')).show();
        }
    });
...
$(document).ready(function () {
    ...
    var target_date = new Date("October 9, 2021").getTime();
